...
public:
    sDOS() {
        ...
        _fileSystem = new FileSystem(_debugger);
        _events = new EventsManager(_debugger);
        _driver_WiFi = new WiFiManager(_debugger, _fileSystem, _events);
    };
    void Setup();
    void Loop();
...
private:
    ...
    //BluetoothDriver & driver_BlueTooth(_debugger, _events);
...
void sDOS::Setup()
{
    ...
    //_drivers.insert(std::make_pair(getDriverID(F("blutoof")), driver_BlueTooth));
    _cpuFrequencyUpdate();
    ...
    _drivers.insert(std::make_pair(getDriverID(F("i2c")), _driver_I2C));
#endif
#ifdef ENABLE_TTP223
...
#define ENABLE_RTC
    SDOS_PCF8563 driver_RTC(_debugger, _events, _driver_I2C));
